...
export default function Products({ file, isPreview, products }) {
  ...
  const formConfig = {
    id: '../../content/products/pasture-raised-liquid-whole-eggs.json',
    initialValues: file,
    label: 'Products Page',
    fields: [
      {
      ...
      <div className={`relative`}>
        <Head>
          ...
          <meta
            name='description'
            content={
              file.data.meta && file.data.meta.excerpt
                ? file.data.meta.excerpt.replace(/(<([^>]+)>)/gi, '')
          ...
          <meta
            ...
            content={
              file.data.meta &&
              file.data.meta.keywords &&
              file.data.meta.keywords.length > 0
                ? file.data.meta.keywords.join()
...
export const getStaticProps = async function ({ preview, previewData }) {
  if (preview) {
    return getGithubPreviewProps({
      ...previewData,
      fileRelativePath:
        'content/products/pasture-raised-liquid-whole-eggs.json',
      parse: parseJson,
      isPreview: true,
    });
  }
